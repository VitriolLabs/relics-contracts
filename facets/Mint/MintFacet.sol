...
contract MintFacet is MintInternal {
     ...
     */
    function createVoucherTokens(address holder, uint16 mintQty, MintVoucher calldata voucher) public payable {
        _createVoucherTokens(holder, mintQty, voucher);
    }

    /**
     * @notice Allows creation of a variable number of NFTs to a chosen holder.
     * @param  holder - recipient of the minted NFTs.
     * @param  mintQty - the chosen quantity of NFTs to mint.
     */
    function createTokens(address holder, uint16 mintQty) public payable onlyAllowedFeature(0, NO_VOUCHER_BIT) {
        MintStorage.Layout storage m = MintStorage.layout();
        _createTokens(m, holder, mintQty, false);
    }
