...
abstract contract MintInternal is BaseStorage, AccessControlInternal, FeatureFlag, MintVoucherVerifier, ERC721BaseInternal, ERC721EnumerableInternal {
    ...
    /**
     * @dev   Make sure user not minting more than allowed qty.
     * @param qty - the number of NFTs to be minted.
     */
    function _ensureCanMintQty(MintStorage.Layout storage m, uint16 qty) internal virtual {
        ...
        // remaining > count >= 1
        if ((qty > (maxPerAddress - mintedPerAddress)) || (qty < 1)) {
            // Cannot mint more than the maximum
            revert MintLimit();
        }
        m._incTokensMintedPerAddress(msg.sender, qty);
    }

    /**
     * @dev   Checks additional features of the voucher to make sure it is being consumed when expected.
     * @param voucherSigner - the signer of the MintVoucher.
     */
    function _ensureVoucherInPhase(MintStorage.Layout storage m, address voucherSigner) internal virtual {
     ...
     */
    function _createVoucherTokens(address holder, uint16 mintQty, MintVoucher calldata voucher) internal virtual {
        ...
        // check if voucher-role allowed in current phase
        _ensureVoucherInPhase(m, voucherSigner);

        _createTokens(m, holder, mintQty, voucher.free == 1);
    }

    /**
     * @dev   Allows creation of a variable number of NFTs to a chosen holder
     * @param holder - recipient of the minted NFTs.
     * @param mintQty - the chosen quantity of NFTs to mint.
     */
    function _createTokens(MintStorage.Layout storage m, address holder, uint16 mintQty, bool free) internal virtual {
        ...
        if (msg.value != (free ? 0 : (_getMintFee(currentPhase, mintQty)))) {
            revert MintFee();
        }

        // check if user allowed to mint based on qty
        _ensureCanMintQty(m, mintQty);
     ...
     */
    function _getMintFee(uint256 currentPhase, uint16 qty) internal view virtual override(MintVoucherVerifier) returns (uint256) {
     ...
     */
    function _getMintPhase() internal view virtual returns (uint16) {
     ...
     */
    function _getMaxTokensPerAddress() internal view virtual returns (uint16 maxTokensPerAddress) {
